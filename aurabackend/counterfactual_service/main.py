...
async def financial_audit_exceptions(record_hash: str,
    ...
    BUG-098: previously took no tenant/auth at all, unlike the sibling
    decision endpoint below — tenant now comes from the verified token,
    same as everywhere else in this file (_ledger_tenant)."""
    from . import exception_queue
    try:
        return await asyncio.to_thread(
            exception_queue.pending_exceptions, record_hash, _ledger_tenant(user))

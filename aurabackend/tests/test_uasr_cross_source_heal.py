"""
Unit tests for ``uasr.cross_source_heal.attempt_cross_source_heal`` -- the
module extracted from ``service.py``'s original ``_attempt_cross_source_heal``
so the HTTP path (service.py) and the Kafka MAPE-K worker (mapek_worker.py)
share one implementation (candidate #5's opt-in cross-source auto-heal).

Deliberately imports ONLY ``uasr.cross_source_heal``, ``uasr.db``, and
``uasr.models`` -- never ``uasr.mapek_worker`` or ``uasr.recovery_loop`` --
per docs/BUG_REGISTRY.md's BUG-008: importing uasr.mapek_worker (which
transitively imports RecoveryLoop -> the reflector/actuator agents -> an
LLM-provider client) alongside a freshly created async DB engine in the same
process has reproducibly hung the interpreter at exit. ``tracker``/``loop``
are passed in as mocks here (their own correctness is covered by
test_uasr_cross_source_correlation.py's unit tests), so this file never needs
a real ``RecoveryLoop`` at all.

Same DB-isolation approach as test_uasr_service_cross_source_heal.py: an
isolated temp-file SQLite DB (the real dev data/metadata.db predates later
UASR migrations and init_uasr_db()'s create_all never ALTERs an existing
table), and the insert + heal-attempt call share ONE get_session() block
because attempt_cross_source_heal commits on the session it's given.
"""
...
from uasr.cross_source_heal import attempt_cross_source_heal  # noqa: E402
...
def _isolated_metadata_db():
    ...
    tmp_path = os.path.join(tempfile.gettempdir(), f"aura_uasr_xheal_test_{uuid.uuid4().hex[:8]}.db")
    ...
    # the full story (reproduced and root-caused there).
...
def _rec_id() -> str:
    return f"xheal_mod_test_{uuid.uuid4().hex[:8]}"
...
class TestAttemptCrossSourceHeal:
    ...
    async def test_no_sibling_shim_leaves_the_record_unchanged(self):
        ...
        tracker = MagicMock()
        loop = MagicMock()
        tracker.find_recent_deployed_shim.return_value = None
        ...
        async for session in get_session():
            ...
            result = await attempt_cross_source_heal(
                _drift(), _batch(), rec, session, tracker, loop, 60.0,
            )
            assert result is None
            loop.run_with_candidate_shim.assert_not_called()
            break
    ...
    async def test_sibling_shim_that_fails_validation_leaves_the_record_unchanged(self):
        ...
        tracker = MagicMock()
        loop = MagicMock()
        tracker.find_recent_deployed_shim.return_value = ("s_sibling", "return rows")
        loop.run_with_candidate_shim = AsyncMock(return_value=RecoveryLoopResult(
    ...
    async def test_successful_cross_heal_updates_the_existing_record(self):
        ...
        async for session in get_session():
            ...
            recorded_event = tracker.record.call_args[0][0]
    ...
    @pytest.mark.asyncio
    async def test_uses_the_passed_in_window_not_a_hardcoded_default(self):
        """Proves the function reads its window from the argument, not a
        module global -- the whole point of the extraction (service.py and
        mapek_worker.py resolve their own window independently)."""
        rec_id = _rec_id()
        tracker = MagicMock()
        loop = MagicMock()
        tracker.find_recent_deployed_shim.return_value = None
        ...
        async for session in get_session():
            ...
            await attempt_cross_source_heal(
                _drift(), _batch(), rec, session, tracker, loop, 12.5,
            )
            tracker.find_recent_deployed_shim.assert_called_once_with(
                DriftType.SCHEMA, "s_failing", 12.5,
            )
            break

"""
TransformAgent async-safety regression test.

TransformAgent._generate_transforms runs under a single uvicorn worker (see
.claude/rules/backend.md "Async safety"). Its LLM call must be offloaded with
asyncio.to_thread so a slow provider round-trip doesn't freeze every
concurrent request. This test proves the event loop stays responsive while
the (synchronous, blocking) LLM call is in flight.
"""
...
def test_transform_agent_does_not_block_event_loop():
    """While TransformAgent awaits its (slow, synchronous) LLM call, a
    ...
    ctx = AgentContext(
        user_prompt="dedupe the customers table",
        task_description="dedupe the customers table",
        schema_context={},
    )
...
def test_transform_agent_generates_via_generate_json():
    ...
    class _FastLLM:
        ...
        def generate_json(self, prompt: str):
            return ["SELECT DISTINCT * FROM raw_data;"]
    ...
    assert result.output.get("transforms") == ["SELECT DISTINCT * FROM raw_data;"]

...
- **Production cleanup:** the ~20 erroneously-created tables in the box's `gateway.db`/`ledger.db` are empty and never queried by the running app (each `Base` only ever connects through its own engine/URL, so orphaned tables from a different Base are inert) — left in place rather than dropped immediately, pending the operator's confirmation on whether to clean them up now or treat them as harmless residue.

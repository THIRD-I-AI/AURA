...
- **Caused by:** none — pre-existing.
- **Fix:** `_pg_pool_key` now folds the password into a keyed HMAC-SHA256 (random per-process secret, `_PG_POOL_KEY_SECRET`) instead of excluding it — a wrong/blank password can never hash to the same key as the real one, so it can never be handed the real pool; `asyncpg.create_pool` then fails naturally against the real server. Using HMAC (not a bare hash) keeps the original CodeQL weak-password-hashing false-flag this exclusion was written to dodge from re-triggering, since a keyed MAC is the standard accepted mitigation for that finding class, not an instance of it. New tests: `test_different_password_different_key`, `test_wrong_password_never_reuses_the_real_pool` (asserts a second call with a wrong password creates a genuinely separate pool and that `asyncpg.create_pool` was actually invoked with the wrong password, would fail against a real server). Confirmed non-vacuous: stashed the fix, both new tests failed exactly as expected (same key, same pool reused); restored, 9/9 pass. Ruff clean (`E,F,I,W`, CI's actual ignore list).
- **Fix:** PR to follow.

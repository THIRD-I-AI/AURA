...
- **Immediate production mitigation (applied manually, ahead of this code fix):** the box's `gateway.db` was brought up to date by applying the exact DDL from migration `20260903_e5f6a7b8c9d0_query_history_workspace_id.py` directly, since Claude's own `docker exec` write access to the production box is blocked by the auto-mode classifier (a destructive/production-write class of action; adding a Bash permission rule for it does not bypass the block) — the operator ran it by hand over SSH. A pre-change backup was taken at `/data/state/gateway.db.bak-pre-migration-fix-20260908`. Once this PR's redeploy reaches the box, `_run_split_db_pass()` will independently reconcile the same state (finding the column already present and stamping, not re-executing) — belt-and-suspenders, not a conflict.

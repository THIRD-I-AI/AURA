...
## BUG-035: etl.py's three source-file reads bypass the S45 StorageBackend abstraction — 404 under AURA_STORAGE_BACKEND=s3
- **Status:** partially fixed — `aurabackend/api_gateway/routers/etl.py` fixed; `aurabackend/api_gateway/routers/pipelines.py`'s piece of the same gap intentionally deferred (see below).
...
- **Severity:** production-gap — `POST /etl/preview-source`, `POST /etl/execute`, and `POST /etl/natural-language` built a local filesystem path via `tenant_upload_dir(request)` + a raw/sandboxed join and read it directly, instead of going through `shared/storage`'s `StorageBackend` abstraction (S45, PR #117) the way `files.py`/`data_utils.py` already do. Under `AURA_STORAGE_BACKEND=local` (the default) this was invisible — the resolved path happened to match where `LocalBackend` also looks. Under `AURA_STORAGE_BACKEND=s3`, `POST /upload` writes the file to S3 only (`files.py:160`), so these three endpoints found nothing on local disk and 404'd on every file a caller had legitimately just uploaded.
...
- **Fix (etl.py):** `etl_preview_source`, `etl_execute`, and `etl_from_natural_language` now resolve the source file via `get_storage_backend().exists(tenant, name)` / `.duckdb_uri(tenant, name)` instead of `safe_join(upload_dir, name)` + `Path.exists()` — the same read pattern `data_utils.py::build_schema_context` already uses, so `smart_load_file` gets an `s3://...` URI under S3 mode (already proven to work by `test_storage_s3_duckdb.py`'s existing DuckDB-httpfs test) or a plain local path under local mode, unchanged. Path-traversal protection moved from `safe_join`/`PathTraversalError` to the backend's own `safe_object_name()` (rejects absolute paths, `..`, separators, NUL) — same guarantee, enforced at the layer the backend itself owns.
  Tests: `aurabackend/tests/test_etl_preview_source_traversal.py` (existing traversal regression, updated to isolate the storage backend via `AURA_UPLOADS_ROOT` + `reset_storage_backend()` instead of monkeypatching `workspaces._UPLOADS_ROOT` directly, which the backend never reads) — 1/1 pass. New `aurabackend/tests/test_etl_storage_backend.py` (Tier A, local mode): `/etl/execute` and `/etl/natural-language` read an uploaded file via the backend and produce correct results; an unknown file 404s cleanly — 3/3 pass. New Tier-B test in `aurabackend/tests/test_storage_s3_duckdb.py` (`test_etl_preview_and_execute_read_uploaded_file_from_s3`, gated on `AURA_S3_TEST_ENDPOINT`, runs in the existing `storage-s3` CI lane against real MinIO): drives `/etl/preview-source` and `/etl/execute` end-to-end through the real HTTP app against an S3-backed upload — this is the actual regression proof for the bug; could not be run locally (no MinIO daemon available in this environment) but will execute in CI.
- **Deferred (pipelines.py's half of the same gap):** `aurabackend/api_gateway/routers/pipelines.py` has the identical local-only pattern (`pipeline_generate`, `pipeline_execute`, `pipeline_execute_async`, `pipeline_file_schema` all thread a raw `upload_dir: str` into `pipeline/generator.py::PipelineGenerator` and `pipeline/engine.py::PipelineEngine`), but fixing it properly is materially larger than etl.py's router-local fix: `upload_dir` is baked into `PipelineGenerator.generate()`/`._discover_files()`/`.get_file_schema()` and `PipelineEngine.execute()`/`._load_file_source()`/`._load_duckdb_source()`'s own public signatures — not just their callers — and those engines are also invoked from non-HTTP paths (the scheduler, `inbound_hooks.py`'s webhook-triggered runs via `default_upload_dir()`). Threading a `tenant`/backend-based read through that surface touches more call sites and needs its own design pass + test coverage across both the request and background-job paths, rather than being folded into this surgical fix. Real gap, `AURA_STORAGE_BACKEND=s3` deployments should not run pipelines against uploaded files until this lands — tracked here as the next piece, not implemented in this pass.

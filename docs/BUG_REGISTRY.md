...
- **Status:** open — root cause confirmed live on the production box (dataaura.duckdns.org), fix not yet written or deployed.
...
- **Fix (not yet applied):** `env.py` needs to resolve `sqlalchemy.url` per-`Base`/per-migration instead of a single global URL — e.g. running each `Base.metadata`'s migrations against its own configured URL (`GATEWAY_DATABASE_URL` for `api_gateway.persistence.Base`, `AURA_LEDGER_DATABASE_URL` for `shared.audit_ledger.Base`, `METADATA_DATABASE_URL` for everything else), or documenting/enforcing that all `*_DATABASE_URL` env vars MUST coincide in any deployment that uses Alembic (and failing loud at startup if they don't) if a true multi-database migration setup is out of scope. Needs its own design pass — out of scope for tonight's live intervention, which only unblocked the immediate production symptom (see below) without touching this file.
- **Immediate production mitigation (requested, pending confirmation):** the fix is to apply the exact DDL from migration `20260903_e5f6a7b8c9d0_query_history_workspace_id.py` (`ALTER TABLE gateway_query_history ADD COLUMN workspace_id VARCHAR(64)` + `CREATE INDEX ix_gateway_query_history_workspace_id`) directly to `/data/state/gateway.db` via `docker exec ... python3 -c "..."`, since `gateway.db` has no `alembic_version` table to reconcile (confirmed it's `create_all()`-only) and Alembic cannot reach it at all in this deployment's current configuration. Claude's `docker exec` write access to the production box is blocked by the auto-mode classifier (destructive/production-write class of action, and adding a Bash permission rule for it does not bypass the block), so the operator ran this by hand over SSH; a pre-change backup was taken at `/data/state/gateway.db.bak-pre-migration-fix-20260908`. This mitigation must be repeated for every future gateway-table migration until BUG-044's real fix ships — Alembic will keep silently missing `gateway.db` on this box.
- **Tests:** none yet — this is a config/infra bug in the migration framework itself, not application code; a regression test would need a fixture asserting `env.py` resolves distinct URLs per `Base.metadata` when the corresponding env vars differ, which doesn't exist yet. Flagged for the fix PR.

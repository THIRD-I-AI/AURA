...
|---|---|---|---|---|
| **Sec-7** | Mounith | `feature/sec7-enterprise-hardening` | 2026-06-11 | Enterprise deployment hardening (issue #74, **PR #75**): Helm pins `ENVIRONMENT=production`+`AURA_AUTH_MODE=password` (the Sec-4 boot gates were never armed on-cluster — open admin-token minting), ERP ingestion 401 fail-closed, PIIMaskingMiddleware rewritten as pure ASGI (the `request._receive` version was a **silent no-op** on current Starlette) + upgraded to keyed tokenization (resolves S34d raw-in-boundary tension), envSecretName contract, ENTERPRISE.md deployment checklist, docs/INVESTOR_DEMO.md. |

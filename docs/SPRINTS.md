...
|---|---|---|---|---|
| **Sec-8** | Mouni | `feature/sec8-upload-path-traversal` | 2026-06-17 | Path-traversal hardening of the file-upload handler — sanitize the client filename (basename + `commonpath` containment) so a name like `../keys/signing_ed25519.pem` can't escape the upload dir (issue #101). First step of pre-multitenant file-storage hardening; per-tenant directory partitioning tracked as a follow-up. |

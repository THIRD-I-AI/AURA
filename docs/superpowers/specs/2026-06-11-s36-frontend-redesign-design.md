# S36 — Enterprise Frontend Redesign: Terminal Authority
...
- **Wizard:** S35 drop-zone restyled onto primitives; `Stepper` replaces the
...
| **Findings** | `/app/findings` | **ExceptionQueue (HITL)** — today orphaned with no route; wired to the real S34b endpoints incl. JWT identity |
...
- **Shell:** new `Sidebar` (6 grouped sections, every entry iconed — S35a's
...
1. **Auth layer:** `ApiClient` gains a token store (memory, not
   localStorage) + `Authorization: Bearer` injection; login via the gateway
   `/auth/token` (open mode in dev); 401 responses surface a sign-in prompt.
   Unlocks the JWT-gated decision endpoint (S34b) from the UI.
...
Branch `feature/s36-terminal-authority` off main after PR #73 (S35) merges.
...
final sweep. Rebase on main at least daily (Mounith's S34d is backend-only;
expected conflicts ≈ none). One squash-merge PR "Land Sprint S36"; sprint
...
- Chat/agent panels have the most bespoke styling; they migrate last and may
  land visually "compatible but not redesigned" in S36, queued for S36.1.
